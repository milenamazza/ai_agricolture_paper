"""
run_laplace_residual_optuna.py
─────────────────────────────────────────────────────────────────────────────
Come run_laplace_optuna.py, ma la rete impara il RESIDUO di Penman-Monteith
invece del valore assoluto di umidità:
...
Le feature restano esclusivamente quelle componibili: Penman compare solo nel
target e nella ricostruzione, mai come input.

Nota sulla calibrazione della precisione del prior: avviene in spazio
RESIDUO, cioè sulla stessa scala su cui la rete è stata allenata. Poiché la
ricostruzione somma una costante e non altera le varianze, il valore scelto
resta ottimale anche in spazio assoluto.

Output in results_residual_optuna/:
  laplace_residual_metrics.csv, laplace_residual_preds.csv,
  laplace_residual_hyperparameters.csv, split_composition.csv

Utilizzo:
    python -u run_laplace_residual_optuna.py
    python -u run_laplace_residual_optuna.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA --n-trials 3
"""
...
MODEL_NAME = "MLP-Laplace-Residual-Optuna"
...
def run_uncertainty_experiments(datasets_penman, fsets, n_trials,
    ...
    for fset_index, fset_name in enumerate(fsets, start=1):
        ...
        # --- Ricerca Optuna via validation NLL post-Laplace (su residuo) ----
        ...
        for split_name, split_df, X_split_imp, Y_split_abs, Y_split_penman in splits_to_evaluate:
            mu_pred_res, epistemic_std, aleatoric_std, total_std = dml.laplace_predict(
...
def main():
    ...
    metrics_path = os.path.join(args.out, "laplace_residual_metrics.csv")
    preds_path = os.path.join(args.out, "laplace_residual_preds.csv")
    hparams_path = os.path.join(args.out, "laplace_residual_hyperparameters.csv")
