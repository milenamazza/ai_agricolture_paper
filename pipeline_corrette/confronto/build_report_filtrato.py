"""
build_report_componibile_filtrato.py
─────────────────────────────────────────────────────────────────────────────
Confronto tra le 3 pipeline "componibili" GIÀ filtrate sul sotto-insieme
comparabile con Penman (vedi run_multioutput_componibile_comparabile_penman.py
e penman_residual_componibile_solo_feature.py):
  - ML puro (filtrato):  multioutput_ml/results_comparabile_penman/experiments_comparabile_penman_preds.csv
  - Penman puro:         penman_baseline_componibile/results/penman_preds.csv
  - Penman + residuo (solo feature, filtrato):
                          penman_residual_componibile/results_solo_feature/penman_residual_solo_feature_preds.csv

Differenza rispetto a build_report_componibile.py: lì "ML puro" e "Penman +
residuo" erano allenati sull'intero split CANONICO e la finestra comune con
Penman veniva ricavata SOLO in fase di confronto (a posteriori). Qui invece
sia ML puro che il residuo sono già stati allenati SOLO sul sotto-insieme di
righe in cui la catena fisica Penman arriva in fondo per tutti e 7 gli
orizzonti (stesso filtro, applicato PRIMA del training, non dopo). Il
residuo usa inoltre la variante "solo feature" (nessuna colonna fisica
Penman in più rispetto a ML puro — colonne componibili identiche).

La logica di "finestra comune" qui sotto (identica a build_report_componibile.py,
non re-inventata) resta come rete di sicurezza/verifica: dato che le tre
fonti condividono già a monte lo stesso filtro Penman, ci si aspetta che
ml_days, res_days e pen_days coincidano quasi esattamente (a meno del
vincolo aggiuntivo di lookback dell'LSTM, che può togliere qualche riga in
più indipendentemente per pipeline) — se la finestra comune risultasse molto
più piccola della copertura nativa di ciascuna fonte sarebbe un segnale di
errore da indagare, non il comportamento atteso.

Output: confronto/confronto_filtrato.xlsx
"""
...
ML_PREDS = os.path.join(PIPE, "multioutput", "results_comparabile_penman", "experiments_comparabile_penman_preds.csv")
PENMAN_PREDS = os.path.join(PIPE, "penman_baseline", "results", "penman_preds.csv")
RESIDUAL_PREDS = os.path.join(PIPE, "penman_residual", "results_solo_feature", "penman_residual_solo_feature_preds.csv")

OUT_PATH = os.path.join(HERE, "confronto_filtrato.xlsx")
...
TIPO_ORDER = ["Penman puro", "ML puro (componibile, filtrato)", "Penman + ML (residuo, solo feature, filtrato)"]
...
def compute_dayset_info(ml, pen, res):
    ...
    counts = {
        "ML puro (componibile, filtrato) — copertura nativa": len(ml_days),
        "Penman puro — copertura nativa": len(pen_days),
        "Penman + residuo (solo feature, filtrato) — copertura nativa": len(res_days),
...
def per_sensor_coverage(ml_days, pen_days, res_days, common_days):
    ...
    for sensor in sensors:
        rows.append({
            "sensore": sensor,
            "ML puro (filtrato) — copertura nativa": sum(1 for s, d in ml_days if s == sensor),
            "Penman puro — copertura nativa": sum(1 for s, d in pen_days if s == sensor),
            "Penman + residuo (filtrato) — copertura nativa": sum(1 for s, d in res_days if s == sensor),
...
def load_all():
    ...
    df_ml = recompute_metrics(ml, "ML puro (componibile, filtrato)", common_days, has_fset=True)
    df_pen = recompute_metrics(pen, "Penman puro", common_days, has_fset=False)
    df_res = recompute_metrics(res, "Penman + ML (residuo, solo feature, filtrato)", common_days, has_fset=True)
...
# ═══════════════════════════════════════════════════════════════════════════
# Stile (stessa palette di build_report_componibile.py, per coerenza visiva)
# ═══════════════════════════════════════════════════════════════════════════
...
SECTION_FILL = {
    ...
    "ML puro (componibile, filtrato)":                       PatternFill("solid", fgColor="C9DAF8"),
    "Penman + ML (residuo, solo feature, filtrato)":         PatternFill("solid", fgColor="FCE5CD"),
}
...
def write_summary_sheet(wb, df_all, counts, n_common, coverage):
    ...
    ws.append(["Confronto pipeline componibili FILTRATE su Penman: Penman puro vs ML puro vs Penman+ML residuo (solo feature)"])
    ws["A1"].font = Font(bold=True, size=14)
    ws.append([])

    ws.cell(row=ws.max_row + 1, column=1,
            value="Nota metodologica: a differenza di confronto_componibile.xlsx, qui ML puro e "
                  "il residuo sono stati allenati SOLO sul sotto-insieme di righe in cui la catena "
                  "fisica Penman arriva in fondo per tutti e 7 gli orizzonti (filtro applicato PRIMA "
                  "del training, non dopo — vedi run_multioutput_componibile_comparabile_penman.py e "
                  "penman_residual_componibile_solo_feature.py). Il residuo usa inoltre solo le "
                  "feature componibili (nessuna colonna fisica Penman aggiuntiva). La finestra comune "
                  "qui sotto resta calcolata come rete di sicurezza (stessa logica di "
                  "confronto_componibile.xlsx): ci si aspetta che coincida quasi esattamente con la "
                  "copertura nativa di ciascuna fonte, dato che condividono già a monte lo stesso "
                  "filtro Penman.").font = Font(italic=True)
    ws.cell(row=ws.max_row + 1, column=1,
            value=f"Copertura nativa: {counts['ML puro (componibile, filtrato) — copertura nativa']} giorni ML puro, "
                  f"{counts['Penman puro — copertura nativa']} giorni Penman puro, "
                  f"{counts['Penman + residuo (solo feature, filtrato) — copertura nativa']} giorni residuo "
