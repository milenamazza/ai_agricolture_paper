# ─────────────────────────────────────────────────────────────────────────────
# COPIA in pipeline_corrette/dataset_aggiornato/ di pipeline_corrette/multioutput/run_multioutput_optuna_dominio.py.
...
"""
run_multioutput_componibile_comparabile_penman_optuna_dominio.py
─────────────────────────────────────────────────────────────────────────────
Come run_multioutput_componibile_comparabile_penman_optuna.py (stessa
pipeline ML, stesso filtro di comparabilità con Penman, stessa ricerca
Optuna con batch_size/event_weight — vedi quel file per la spiegazione
completa), ma le feature ML componibili sono UNITE alle colonne fisiche
Penman per orizzonte — stesso identico schema di
penman_residual_componibile/run_penman_residual_componibile_optuna.py
(vedi quel file per la spiegazione dettagliata).

Colonne di dominio aggiunte per orizzonte: penman_kc, penman_etc_pct,
penman_ks, penman_rain_eff_pct, penman_irr_eff_pct, penman_perc_pct,
penman_irrigation_planned (7 su 8 di penman_phys_cols), più
penman_residual_lag1 — ESCLUSA penman_pred_t{h}.

Perché escludere penman_pred_t{h} anche qui, pur non essendoci la stessa
circolarità del file residual (lì costruisce il target e la ricostruzione;
qui il target è ASSOLUTO, Penman entra solo nel filtro di comparabilità,
mai nel target): scelta esplicita dell'utente per restare con l'IDENTICO
insieme di colonne di dominio del file residual — stesso confronto,
stessa lista di feature aggiuntive in entrambe le pipeline "con dominio".

Si verifica esplicitamente (e si stampa per trasparenza) che le colonne di
dominio non si sovrappongano già alle feature ML componibili.

Riuso in sola lettura (nessuna modifica ai file sorgenti):
  - modelli singoli/penman_residual/run_penman_residual.py:
    enrich_all_with_penman_ext, penman_phys_cols — fisica INVARIATA (serve
    per le colonne fisiche intermedie, che la fisica "base" non calcola).
...
Output in results_comparabile_penman_optuna_dominio/ (cartella separata,
per non confondere con la versione senza colonne di dominio):
  experiments_comparabile_penman_optuna_dominio_metrics.csv
  experiments_comparabile_penman_optuna_dominio_preds.csv
  experiments_comparabile_penman_optuna_dominio_hyperparameters.csv
  split_composition.csv

Utilizzo:
    python -u run_multioutput_componibile_comparabile_penman_optuna_dominio.py
    python -u run_multioutput_componibile_comparabile_penman_optuna_dominio.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA TERRA --no-lstm --n-trials 3
"""
...
from penman_ext import enrich_all_with_penman_ext, penman_phys_cols  # noqa: E402  (import in sola lettura, invariato)
import deep_models_multi as dmm  # noqa: E402  (import in sola lettura, invariato)

OUT_DIR = os.path.join(HERE, "results_comparabile_penman_optuna_dominio")

EVENT_WEIGHT = 3
LOOKBACK = 14
TARGET_COLS = [f"target_t{h}" for h in HORIZONS]
PENMAN_PRED_COLS = [f"penman_pred_t{h}" for h in HORIZONS]
PRED_COLS_SET = set(PENMAN_PRED_COLS)
...
# ═══════════════════════════════════════════════════════════════════════════
# Feature: ML componibili + colonne fisiche Penman per orizzonte, ESCLUSA
# penman_pred_t{h} — stessa esatta lista di colonne di dominio del file
# residual (run_penman_residual_componibile_optuna.py).
# ═══════════════════════════════════════════════════════════════════════════

def domain_columns_for(fset_name):
    """Unione delle feature ML del gruppo componibile (su tutti gli
    orizzonti) con le colonne fisiche Penman per orizzonte — TRANNE
    penman_pred_t{h} — e il residuo del giorno precedente. Verifica e
    stampa esplicitamente se le colonne di dominio si sovrappongono già
    alle feature ML componibili (atteso: nessuna sovrapposizione,
    compose_feature_set non include colonne Penman — ma verificato, non
    assunto)."""
    ...
    domain_columns = set()
    for h in HORIZONS:
        domain_columns.update(c for c in penman_phys_cols(h) if c not in PRED_COLS_SET)
    domain_columns.add("penman_residual_lag1")

    overlap = ml_columns & domain_columns
    if overlap:
        print(f"    ATTENZIONE: {len(overlap)} colonne di dominio già presenti tra le feature ML "
              f"componibili (deduplicate, non contate due volte): {sorted(overlap)}")
    else:
        print(f"    Nessuna sovrapposizione tra le {len(ml_columns)} feature ML componibili e le "
              f"{len(domain_columns)} colonne di dominio aggiunte (verificato).")

    return sorted(ml_columns | domain_columns)
...
def run_experiments(datasets_penman, fsets, n_trials, run_mlp=True, run_lstm=True,
    ...
    for fi, fset_name in enumerate(fsets, start=1):
        ...
        domain_columns = domain_columns_for(fset_name)
        available_features = sorted(c for c in domain_columns if c in train_df.columns)
        ...
        print(f"  n_train={len(train_df)} n_val={len(val_df)} n_test={len(test_df)}  "
              f"n_features={len(available_features)}")
...
def main():
    ...
    print("\nCarico configurazione agronomica (colture.csv, sensori-corretti.csv) e simulo "
          "Penman-Monteith esteso (con intermedi per feature di dominio)...")
    ...
    datasets_penman = enrich_all_with_penman_ext(datasets, sensori_df, colture_df, args.alpha_rain, args.alpha_irr)
    ...
    mm_path = os.path.join(args.out, "experiments_comparabile_penman_optuna_dominio_metrics.csv")
    mp_path = os.path.join(args.out, "experiments_comparabile_penman_optuna_dominio_preds.csv")
    hp_path = os.path.join(args.out, "experiments_comparabile_penman_optuna_dominio_hyperparameters.csv")
