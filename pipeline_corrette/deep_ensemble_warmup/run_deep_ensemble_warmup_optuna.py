# ─────────────────────────────────────────────────────────────────────────────
# COPIA CON WARM-UP di pipeline_corrette/deep_ensemble/run_deep_ensemble_optuna.py.
...
"""
run_ensemble_componibile_optuna.py
─────────────────────────────────────────────────────────────────────────────
Stessa pipeline di run_ensemble_componibile.py (stesse feature componibili,
stessa irrigazione ricostruita, stesso split canonico condiviso, stesso Deep
Ensemble a N=5 reti — vedi quel file per la spiegazione completa), ma la
ricerca degli iperparametri usa OPTUNA invece della griglia esaustiva
ParameterGrid. Vedi run_uncertainty_componibile_optuna.py per la motivazione
completa (griglia attuale già a 60 combinazioni; aggiungere batch_size ed
event_weight come dimensioni extra la farebbe esplodere ulteriormente,
mentre Optuna esplora uno spazio più ampio con un budget FISSO di prove).

Come nell'originale, il tuning usa UNA sola rete (seed=dme.BASE_SEED, non
l'intero ensemble): allenare N=5 reti per OGNI prova Optuna moltiplicherebbe
per 5 un costo già alto — si cerca la combinazione migliore con una rete
sola, poi si allenano N=5 reti indipendenti con QUELLA combinazione, una per
seed, esattamente come nell'originale.
...
volta con l'event_weight migliore trovato per allenare i 5 membri finali.
...
Output in results_optuna/ (cartella separata da results/):
  ensemble_metrics.csv, ensemble_preds.csv, ensemble_hyperparameters.csv,
  split_composition.csv — stesso schema dell'originale, più event_weight tra
  gli iperparametri salvati.

Utilizzo:
    python -u run_ensemble_componibile_optuna.py
    python -u run_ensemble_componibile_optuna.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA TERRA --n-trials 15
"""
...
import deep_models_ensemble_warmup as dme  # noqa: E402  (training con warm-up della media)
...
MODEL_NAME = "MLP-Gaussian-DeepEnsemble-Warmup-Optuna"
N_ENSEMBLE_MEMBERS = 5
N_TRIALS_DEFAULT = 100
OPTUNA_SEED = 42

NN_FIXED_ENSEMBLE = {"patience": 30, "epochs": 300}   # batch_size lo sceglie Optuna, non è più fisso qui
...
# ═══════════════════════════════════════════════════════════════════════════
# Ricerca iperparametri con Optuna (sostituisce dme.tune_mlp_gaussian_multi_val)
# — sempre su UNA sola rete, seed=dme.BASE_SEED, come nell'originale.
# ═══════════════════════════════════════════════════════════════════════════
...
def make_objective(train_df, X_train_imp, Y_train_full, X_val_imp, Y_val_full):
    def objective(trial):
        ...
        _, _, _, val_nll = dme.fit_mlp_gaussian_multi(
            X_train_imp, Y_train_full, X_val_imp, Y_val_full, seed=dme.BASE_SEED,
            sample_weight=sample_weight,
            hidden=HIDDEN_CHOICES[hidden_key], dropout=dropout, lr=lr, batch_size=batch_size,
            patience=NN_FIXED_ENSEMBLE["patience"], epochs=NN_FIXED_ENSEMBLE["epochs"],
        )
        return val_nll
...
# ═══════════════════════════════════════════════════════════════════════════
# Bookkeeping (identico a run_ensemble_componibile.py)
# ═══════════════════════════════════════════════════════════════════════════
...
def run_ensemble_experiments(datasets, fsets, n_trials,
    ...
    for fset_index, fset_name in enumerate(fsets, start=1):
        ...
        print(f"  ricerca Optuna (rete singola): {n_trials} prove (hidden/lr/dropout/batch_size/event_weight)")

        # --- Ricerca Optuna con UNA sola rete (seed di base) ----------------
        ...
        fit_kwargs = {k: v for k, v in best_params.items() if k != "event_weight"}
        ...
        for member_index in range(n_members):
            ...
            model, feature_scaler, y_stats, member_val_nll = dme.fit_mlp_gaussian_multi(
                ...
                sample_weight=sample_weight, **fit_kwargs, **NN_FIXED_ENSEMBLE,
            )
        ...
        # diagnostica del warm-up di ogni membro (epoche e punteggi delle due
        # fasi), salvata dentro la colonna JSON params: lo schema del CSV non cambia
        params_salvati = {**best_params,
                          "warmup_membri": [m.warmup_info for m, _, _ in trained_members]}
        record_hyperparameters(hparam_rows, fset_name, params_salvati, best_val_nll,
...
def main():
    ...
    metrics_path = os.path.join(args.out, "ensemble_warmup_metrics.csv")
    preds_path = os.path.join(args.out, "ensemble_warmup_preds.csv")
    hparams_path = os.path.join(args.out, "ensemble_warmup_hyperparameters.csv")
