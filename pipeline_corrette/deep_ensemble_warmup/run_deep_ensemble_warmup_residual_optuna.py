# ─────────────────────────────────────────────────────────────────────────────
# COPIA CON WARM-UP di pipeline_corrette/deep_ensemble/run_deep_ensemble_residual_optuna.py.
...
"""
run_ensemble_residual_componibile_optuna.py
─────────────────────────────────────────────────────────────────────────────
Come run_ensemble_residual_componibile.py (Deep Ensemble a N=5 reti che
predice il RESIDUO target-penman_pred invece del target assoluto, poi
ricostruisce sommando penman_pred — vedi
uncertainty_residual_componibile/run_uncertainty_residual_componibile.py
per la giustificazione matematica completa), ma la ricerca degli
iperparametri usa OPTUNA invece della griglia esaustiva — stessa motivazione
di run_ensemble_componibile_optuna.py.

Come nell'originale, il tuning usa UNA sola rete (seed=dme.BASE_SEED, non
l'intero ensemble); il target durante il tuning è il RESIDUO.
...
Output in results_optuna/:
  ensemble_residual_metrics.csv, ensemble_residual_preds.csv,
  ensemble_residual_hyperparameters.csv, split_composition.csv

Utilizzo:
    python -u run_ensemble_residual_componibile_optuna.py
    python -u run_ensemble_residual_componibile_optuna.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA TERRA --n-trials 15
"""
...
MODEL_NAME = "MLP-Gaussian-DeepEnsemble-Residual-Warmup-Optuna"
...
# ═══════════════════════════════════════════════════════════════════════════
# Ricerca iperparametri con Optuna — sempre su UNA sola rete (seed di base),
# obiettivo calcolato in spazio RESIDUO.
# ═══════════════════════════════════════════════════════════════════════════
...
def make_objective(train_df, X_train_imp, Y_train_res, X_val_imp, Y_val_res):
    def objective(trial):
        ...
        _, _, _, val_nll = dme.fit_mlp_gaussian_multi(
            X_train_imp, Y_train_res, X_val_imp, Y_val_res, seed=dme.BASE_SEED,
...
# ═══════════════════════════════════════════════════════════════════════════
# Bookkeeping — IDENTICA a run_ensemble_residual_componibile.py.
# ═══════════════════════════════════════════════════════════════════════════
...
def run_ensemble_experiments(datasets_penman, fsets, n_trials,
    ...
    for fset_index, fset_name in enumerate(fsets, start=1):
        ...
        # --- Ricerca Optuna con UNA sola rete (seed di base), target=residuo -
        ...
        # --- Training di N reti indipendenti (residuo) con quegli iperparametri -
...
def main():
    ...
    metrics_path = os.path.join(args.out, "ensemble_warmup_residual_metrics.csv")
    preds_path = os.path.join(args.out, "ensemble_warmup_residual_preds.csv")
    hparams_path = os.path.join(args.out, "ensemble_warmup_residual_hyperparameters.csv")
