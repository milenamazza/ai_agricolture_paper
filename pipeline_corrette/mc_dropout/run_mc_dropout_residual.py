"""
run_uncertainty_residual_componibile.py
─────────────────────────────────────────────────────────────────────────────
Come uncertainty_componibile/run_uncertainty_componibile.py (MC-Dropout per
l'incertezza epistemica + testa eteroscedastica per l'aleatoria — vedi
deep_models_uncertainty.py per la matematica, INVARIATA qui), ma il modello
predice il RESIDUO (target - penman_pred) invece del target assoluto, come
penman_residual_componibile fa per Ridge/RF/MLP/LSTM — qui applicato per la
prima volta anche a un modello di INCERTEZZA.

Perché è corretto sommare la previsione Penman DOPO senza ricalcolare
l'incertezza: penman_pred è un valore FISSO (simulazione deterministica,
nessuna componente aleatoria). Se il modello predice il residuo come
res ~ N(mu_res, sigma^2), allora assoluto = res + penman_pred ~
N(mu_res + penman_pred, sigma^2) — sommare una costante sposta solo la
media, la deviazione standard (epistemica/aleatoria/totale) non cambia.
Si trasla quindi SOLO mu_pred, mai epistemic_std/aleatoric_std/total_std.

Differenza dalle feature di penman_residual_componibile: qui le feature ML
restano ESCLUSIVAMENTE quelle componibili (feature_groups_util) — NESSUNA
colonna fisica Penman come input al modello. Penman entra solo per costruire
il target residuo e per ricostruire la previsione assoluta a fine
predizione, non come feature.

Riuso in sola lettura (nessuna modifica ai file sorgenti):
  - uncertainty_ml/deep_models_uncertainty.py, eval_uncertainty.py: INVARIATI.
  - penman_baseline/run_penman.py: enrich_all_with_penman, INVARIATA — stessa
    fisica già riusata in run_uncertainty_componibile_optuna.py per il
    filtro di comparabilità.
  - modelli singoli/data_driven_ml/utils.py, run_experiments.py: come nella
    pipeline diretta, più SAT_OVERRIDE/load_colture/load_sensori/
    apply_soil_overrides.
  - ai_feature_componibile/{feature_groups_util.py,dati_ricostruiti_util.py}.

Il filtro sul sottoinsieme comparabile con Penman (righe con TUTTI i
penman_pred_t{h} validi) qui è STRUTTURALMENTE necessario, non solo una
scelta di comparabilità: senza penman_pred non esiste il target residuo.

Output in results/:
  uncertainty_residual_metrics.csv
  uncertainty_residual_preds.csv
  uncertainty_residual_hyperparameters.csv
  split_composition.csv

Utilizzo:
    python -u run_uncertainty_residual_componibile.py
    python -u run_uncertainty_residual_componibile.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA TERRA
"""
...
MODEL_NAME = "MLP-Gaussian-MCDropout-Residual"

NN_FIXED_UQ = {"batch_size": 32, "patience": 30, "epochs": 300}
GRID_MLP_UQ = {
...
# + aggregato "ALL" — IDENTICA a run_uncertainty_componibile.py. Riceve
# sempre valori in spazio ASSOLUTO (già ricostruiti sommando penman_pred),
# quindi coverage/sharpness/NLL sono confrontabili con le altre pipeline.
# ═══════════════════════════════════════════════════════════════════════════
...
def run_uncertainty_experiments(datasets_penman, fsets, event_weight=EVENT_WEIGHT,
    ...
    for fset_index, fset_name in enumerate(fsets, start=1):
        ...
        # --- Grid search via validation NLL (target = residuo) -------------
        (best_model, feature_scaler, y_stats), best_params, best_val_nll = dmu.tune_mlp_gaussian_multi_val(
            X_train_imp, Y_train_res, X_val_imp, Y_val_res,
            GRID_MLP_UQ, sample_weight=sample_weight, nn_fixed=NN_FIXED_UQ,
        )
        ...
        for split_name, split_df, X_split_imp, Y_split_abs, Y_split_penman in splits_to_evaluate:
            ...
            # Ricostruzione: sommare una costante (penman_pred) sposta solo
            # la media, le tre deviazioni standard restano quelle del residuo.
...
def main():
    ...
    metrics_path = os.path.join(args.out, "uncertainty_residual_metrics.csv")
    preds_path = os.path.join(args.out, "uncertainty_residual_preds.csv")
    hparams_path = os.path.join(args.out, "uncertainty_residual_hyperparameters.csv")
