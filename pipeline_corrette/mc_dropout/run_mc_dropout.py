"""
run_uncertainty_componibile.py
─────────────────────────────────────────────────────────────────────────────
Come uncertainty_ml/run_uncertainty.py (MC-Dropout per l'incertezza
epistemica + testa eteroscedastica per l'aleatoria, un solo modello MLP
multi-output per feature-set — vedi deep_models_uncertainty.py per la
matematica, INVARIATA qui), ma:
  ...
  3. Lo split è quello CANONICO CONDIVISO
     ...
     di multioutput_ml/, penman_baseline_componibile/ e
     penman_residual_componibile/. Estende a questa pipeline la garanzia
     "stesso split ovunque" già raggiunta per le altre.

Riuso in sola lettura (nessuna modifica ai file sorgenti):
  - uncertainty_ml/deep_models_uncertainty.py, eval_uncertainty.py:
    INVARIATI — la logica MC-Dropout/NLL gaussiana non cambia, cambia solo
    la sorgente di feature/irrigazione/split a monte.
...
Output in results/:
  uncertainty_metrics.csv        — per feature-set x orizzonte-o-ALL x split
  uncertainty_preds.csv          — predizioni giorno per giorno con
                                   epistemic_std/aleatoric_std/total_std
  uncertainty_hyperparameters.csv
  split_composition.csv          — giorni di pioggia/irrigazione per split

Utilizzo:
    python -u run_uncertainty_componibile.py
    python -u run_uncertainty_componibile.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA TERRA
"""
...
MODEL_NAME = "MLP-Gaussian-MCDropout"
...
def run_uncertainty_experiments(datasets, fsets, event_weight=EVENT_WEIGHT,
    ...
    for fset_index, fset_name in enumerate(fsets, start=1):
        ...
        # --- Grid search via validation NLL --------------------------------
