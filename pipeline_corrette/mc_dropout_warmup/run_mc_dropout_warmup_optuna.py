# ─────────────────────────────────────────────────────────────────────────────
# COPIA CON WARM-UP di pipeline_corrette/mc_dropout/run_mc_dropout_optuna.py.
...
"""
run_uncertainty_componibile_optuna.py
─────────────────────────────────────────────────────────────────────────────
Stessa pipeline di run_uncertainty_componibile.py (stesse feature componibili,
stessa irrigazione ricostruita, stesso split canonico condiviso, stesso
MC-Dropout — vedi quel file per la spiegazione completa), ma la ricerca degli
iperparametri usa OPTUNA invece della griglia esaustiva ParameterGrid.

Perché: la griglia attuale (5 hidden x 4 lr x 3 dropout = 60 combinazioni)
esplora già molto, ma aggiungere altre due dimensioni interessanti da provare
— batch_size ed event_weight (il peso dato ai giorni di pioggia/irrigazione
in fase di training, oggi fisso) — moltiplicherebbe ulteriormente il costo
(es. x3 batch_size x5 event_weight = 900 combinazioni x 7 feature-set).
Optuna esplora lo stesso spazio (più ampio, con range continui invece di
pochi valori fissi) con un budget FISSO di prove (--n-trials, default 40),
usando un sampler Bayesiano (TPE) che concentra le prove successive vicino
alle combinazioni che hanno funzionato meglio finora — tipicamente arriva a
un punteggio comparabile o migliore con una frazione delle prove di una
griglia esaustiva.

Cosa NON si tocca: la matematica MC-Dropout/NLL gaussiana resta quella di
deep_models_uncertainty.py (import in sola lettura, invariato) — Optuna
sostituisce SOLO il ciclo di ricerca (l'equivalente di
dmu.tune_mlp_gaussian_multi_val), chiamando dmu.fit_mlp_gaussian_multi
direttamente con i valori suggeriti a ogni prova. Niente pruning
(richiederebbe modificare _train_loop_gaussian per riportare la NLL
intermedia epoca per epoca — fuori dal perimetro "sola lettura" di questo
progetto): ogni prova allena fino a convergenza/early-stopping come nella
griglia originale, solo con un numero di prove molto minore.

event_weight ora è un iperparametro cercato da Optuna (non più un valore
fisso passato da riga di comando): sample_weight viene ricalcolato a ogni
prova con l'event_weight suggerito in quella prova.
...
Output in results_optuna/ (cartella separata da results/, per non confondere
i risultati della griglia esaustiva con quelli di Optuna):
  uncertainty_metrics.csv, uncertainty_preds.csv, uncertainty_hyperparameters.csv,
  split_composition.csv — stesso schema di run_uncertainty_componibile.py, più
  event_weight tra gli iperparametri salvati.

Utilizzo:
    python -u run_uncertainty_componibile_optuna.py
    python -u run_uncertainty_componibile_optuna.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA TERRA --n-trials 15
"""
...
MODEL_NAME = "MLP-Gaussian-MCDropout-Warmup-Optuna"
N_MC_SAMPLES = 100
...
# ═══════════════════════════════════════════════════════════════════════════
# Ricerca iperparametri con Optuna (sostituisce dmu.tune_mlp_gaussian_multi_val)
# ═══════════════════════════════════════════════════════════════════════════
...
def tune_with_optuna(train_df, X_train_imp, Y_train_full, X_val_imp, Y_val_full, n_trials):
    ...
    stringa a tupla, pronto per essere ripassato a fit_mlp_gaussian_multi."""
...
# + aggregato "ALL" — identico a run_uncertainty_componibile.py
# ═══════════════════════════════════════════════════════════════════════════
...
def run_uncertainty_experiments(datasets, fsets, n_trials,
    ...
    print("\nCostruisco lo split canonico (una sola volta, modulo condiviso "
          ...
          "multioutput_ml/, penman_baseline_componibile/ e "
    ...
    for fset_index, fset_name in enumerate(fsets, start=1):
        ...
        # --- MC-Dropout predict + metriche, su train/val/test --------------
        splits_to_evaluate = [
        ...
        for split_name, split_df, X_split_imp, Y_split in splits_to_evaluate:
            mu_pred, epistemic_std, aleatoric_std, total_std = dmu.mc_predict_mlp_gaussian_multi(
...
def main():
    ...
    metrics_path = os.path.join(args.out, "uncertainty_warmup_metrics.csv")
    preds_path = os.path.join(args.out, "uncertainty_warmup_preds.csv")
    hparams_path = os.path.join(args.out, "uncertainty_warmup_hyperparameters.csv")
