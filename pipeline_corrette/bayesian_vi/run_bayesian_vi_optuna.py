"""
run_bayesian_vi_optuna.py
─────────────────────────────────────────────────────────────────────────────
Rete neurale BAYESIANA con variational inference (Bayes by Backprop) sulle
feature componibili, target ASSOLUTO, ricerca degli iperparametri con Optuna.
...
stima l'incertezza:
  ...
  MC-Dropout   pesi fissi, rumore iniettato spegnendo neuroni a caso
  Bayesian VI  ogni peso è una distribuzione appresa, campionata in previsione

La matematica sta in core/deep_models_bayesian_vi.py (import in sola lettura),
che chiude con la STESSA quadrupla (mu_pred, epistemic_std, aleatoric_std,
total_std) delle altre pipeline: i CSV prodotti qui hanno quindi lo schema
identico a quelli di mc_dropout/, deep_ensemble/ e si confrontano riga per
riga.

IPERPARAMETRI CERCATI
hidden, lr, batch_size, event_weight come nelle altre pipeline, più due
specifici del metodo:
  prior_sigma  larghezza del prior N(0, prior_sigma^2) sui pesi. Stretto =
               rete più regolarizzata e meno incerta; largo = più libertà e
               incertezza epistemica maggiore.
  kl_weight    quanto pesa il termine KL nell'ELBO. A 1.0 è l'ELBO "puro"; più
               basso avvicina la rete a una rete deterministica (la sigma sui
               pesi si stringe), più alto la spinge verso il prior.
Entrambi campionati log-uniformi: sono parametri di scala, dove conta l'ordine
di grandezza e non la differenza assoluta.

NIENTE dropout fra gli iperparametri: qui il rumore sui pesi svolge già il
ruolo di regolarizzatore stocastico, e sovrapporre i due meccanismi renderebbe
impossibile attribuire l'incertezza epistemica all'uno o all'altro.

Output in results_optuna/:
  bayesian_vi_metrics.csv, bayesian_vi_preds.csv,
  bayesian_vi_hyperparameters.csv, split_composition.csv

Utilizzo:
    python -u run_bayesian_vi_optuna.py
    python -u run_bayesian_vi_optuna.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA --n-trials 3
"""
...
import deep_models_bayesian_vi as dbv      # noqa: E402  (import in sola lettura)
...
MODEL_NAME = "MLP-BayesianVI-Optuna"
N_VI_SAMPLES = 100          # stesso numero di campioni di MC-Dropout, per confrontabilità
N_TRIALS_DEFAULT = 100
OPTUNA_SEED = 42

NN_FIXED_UQ = {"patience": 30, "epochs": 300}   # batch_size lo sceglie Optuna

# Spazio di ricerca. hidden è mappato su chiavi stringa (invece di passare
# tuple direttamente a suggest_categorical) per restare nel sottoinsieme di
# tipi che Optuna serializza senza ambiguità in ogni backend di storage.
HIDDEN_CHOICES = {
    "32": (32,),
    "64_32": (64, 32),
    "128_64_32": (128, 64, 32),
...
PRIOR_SIGMA_RANGE = (0.01, 1.0)     # log-uniforme
KL_WEIGHT_RANGE = (1e-6, 1.0)       # log-uniforme
# Il limite inferiore è basso di proposito: la KL è una somma su MIGLIAIA di
# pesi divisa per poche centinaia di righe di training, quindi parte già ~100
# volte più grande della NLL. Senza la possibilità di scendere molto sotto 1
# la rete verrebbe spinta sul prior in ogni prova.
...
def make_objective(train_df, X_train_imp, Y_train_full, X_val_imp, Y_val_full):
    """Costruisce la funzione obiettivo per QUESTO feature-set (chiude sui
    dati già preparati, che non cambiano da una prova Optuna all'altra —
    solo gli iperparametri suggeriti cambiano)."""
    ...
    def objective(trial):
        ...
        prior_sigma = trial.suggest_float("prior_sigma", *PRIOR_SIGMA_RANGE, log=True)
        kl_weight = trial.suggest_float("kl_weight", *KL_WEIGHT_RANGE, log=True)

        sample_weight = _make_sample_weights(train_df, event_weight)
        _, _, _, val_nll = dbv.fit_mlp_bayesian_multi(
            X_train_imp, Y_train_full, X_val_imp, Y_val_full,
            sample_weight=sample_weight,
            hidden=HIDDEN_CHOICES[hidden_key], prior_sigma=prior_sigma, kl_weight=kl_weight,
...
def tune_with_optuna(train_df, X_train_imp, Y_train_full, X_val_imp, Y_val_full, n_trials):
    ...
    stringa a tupla, pronto per essere ripassato a fit_mlp_bayesian_multi."""
    ...
    def trial_callback(study, trial):
        print(f"    [Optuna {trial.number + 1}/{n_trials}] val_NLL={trial.value:.4f}  "
              ...
              f"prior_sigma={trial.params['prior_sigma']:.4f} "
              f"kl_weight={trial.params['kl_weight']:.5f}")
...
def run_uncertainty_experiments(datasets, fsets, n_trials,
    ...
    # Stesso identico filtro delle altre pipeline: scarta le righe senza TUTTI
    # i penman_pred_t{h} validi. Le colonne Penman non dipendono dal
    # feature-set ML, quindi il filtro si applica UNA volta qui, non dentro il
    # loop per feature-set.
    ...
    for fset_index, fset_name in enumerate(fsets, start=1):
        ...
        print(f"  ricerca Optuna: {n_trials} prove (hidden/lr/batch_size/event_weight/prior_sigma/kl_weight)")

        # --- Ricerca Optuna via validation NLL ------------------------------
        ...
        best_model, feature_scaler, y_stats, _ = dbv.fit_mlp_bayesian_multi(
            X_train_imp, Y_train_full, X_val_imp, Y_val_full,
            sample_weight=sample_weight,
            hidden=best_params["hidden"], prior_sigma=best_params["prior_sigma"],
            kl_weight=best_params["kl_weight"],
            lr=best_params["lr"], batch_size=best_params["batch_size"],
            patience=NN_FIXED_UQ["patience"], epochs=NN_FIXED_UQ["epochs"],
        )

        # --- Predizione campionando i pesi + metriche, su train/val/test ----
        splits_to_evaluate = [
        ...
        for split_name, split_df, X_split_imp, Y_split in splits_to_evaluate:
            mu_pred, epistemic_std, aleatoric_std, total_std = dbv.vi_predict_mlp_bayesian_multi(
                best_model, feature_scaler, y_stats, X_split_imp, n_samples=n_vi_samples,
            )
            record_results(
...
def main():
    ...
    parser.add_argument("--vi-samples", type=int, default=N_VI_SAMPLES,
                        help="Numero di set di pesi campionati in predizione")
    ...
    metrics_df, preds_df, hparams_df, split_composition = run_uncertainty_experiments(
        ...
        n_vi_samples=args.vi_samples,
    )

    metrics_path = os.path.join(args.out, "bayesian_vi_metrics.csv")
    preds_path = os.path.join(args.out, "bayesian_vi_preds.csv")
    hparams_path = os.path.join(args.out, "bayesian_vi_hyperparameters.csv")
