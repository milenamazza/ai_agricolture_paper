"""
run_bayesian_vi_residual_optuna.py
─────────────────────────────────────────────────────────────────────────────
Come run_bayesian_vi_optuna.py, ma la rete bayesiana impara il RESIDUO di
Penman-Monteith invece del valore assoluto di umidità:

    target residuo = target_t{h} - penman_pred_t{h}

e la previsione viene riportata in scala assoluta sommando di nuovo la
previsione fisica:

    mu_pred_abs = mu_pred_res + penman_pred

Perché le tre deviazioni standard NON si toccano nella ricostruzione:
penman_pred è una quantità deterministica, nota senza incertezza al momento
della previsione. Sommare una costante a una variabile casuale ne trasla la
media ma ne lascia invariata la varianza — quindi epistemic_std, aleatoric_std
e total_std calcolate in spazio residuo sono già quelle giuste in spazio
assoluto.

Il filtro sulle righe con penman_pred_t{h} valido qui è STRUTTURALMENTE
necessario (non solo per comparabilità): senza penman_pred non esiste il
target residuo.

Le feature restano esclusivamente quelle componibili: nessuna colonna fisica
di Penman entra come input: Penman compare solo nel target e nella
ricostruzione.

Output in results_residual_optuna/:
  bayesian_vi_residual_metrics.csv, bayesian_vi_residual_preds.csv,
  bayesian_vi_residual_hyperparameters.csv, split_composition.csv

Utilizzo:
    python -u run_bayesian_vi_residual_optuna.py
    python -u run_bayesian_vi_residual_optuna.py --sensors-filter EM-500-9 EM-500-12 --fsets ACQUA --n-trials 3
"""
...
OUT_DIR = os.path.join(HERE, "results_residual_optuna")   # diretta e residuo: cartelle separate
...
MODEL_NAME = "MLP-BayesianVI-Residual-Optuna"
N_VI_SAMPLES = 100
...
# ═══════════════════════════════════════════════════════════════════════════
# Bookkeeping — riceve sempre valori in spazio ASSOLUTO (già ricostruiti
# sommando penman_pred), quindi è identica alla versione diretta
# ═══════════════════════════════════════════════════════════════════════════
...
def run_uncertainty_experiments(datasets_penman, fsets, n_trials,
    ...
    for fset_index, fset_name in enumerate(fsets, start=1):
        ...
        # --- Predizione (residuo) + ricostruzione assoluta ------------------
        splits_to_evaluate = [
        ...
        for split_name, split_df, X_split_imp, Y_split_abs, Y_split_penman in splits_to_evaluate:
            mu_pred_res, epistemic_std, aleatoric_std, total_std = dbv.vi_predict_mlp_bayesian_multi(
                best_model, feature_scaler, y_stats, X_split_imp, n_samples=n_vi_samples,
            )
            mu_pred_abs = mu_pred_res + Y_split_penman   # le std restano quelle del residuo: vedi docstring
            record_results(
...
def main():
    ...
    metrics_path = os.path.join(args.out, "bayesian_vi_residual_metrics.csv")
    preds_path = os.path.join(args.out, "bayesian_vi_residual_preds.csv")
    hparams_path = os.path.join(args.out, "bayesian_vi_residual_hyperparameters.csv")
